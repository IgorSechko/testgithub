...
#                     default='configs/env.yml')
# parser.add_argument('--config_exp',
#                     help='Config file for the experiment',
#                     default='configs/pretext/simclr_custom.yml')
...
from utils.common_config import get_val_dataset, get_val_transformations, get_val_dataloader, \
    get_model, get_train_dataset, get_train_dataloader, get_train_transformations
...
FLAGS.add_argument('--config_exp', help='Location of config file', default="configs/scan/scan_custom.yml")
FLAGS.add_argument('--model', help='Location where model is saved',
                   default="/home/ITRANSITION.CORP/i.sechko/Downloads/scan_cifar-10.pth.tar")
...
def visualize_indices_subplots(indices, dataset, name):
    import matplotlib.pyplot as plt
    import numpy as np
    side = 5
    ...
    # plt.show()
    plt.savefig(fname=f"visualized/{name}.png")
    plt.close(fig=fig)
...
if __name__ == "__main__":
    ...
    if config['setup'] in ['simclr', 'moco']:
        ...
        clustering_stats, reordered_preds_cpu = hungarian_evaluate(head, predictions, dataset.classes,
                                                                   compute_confusion_matrix=True)
#         print(clustering_stats)
#         torch.save(features, "tensors/footwear_features.pt")
#         # from annoy_utils import build_annoy_trees
#         # build_annoy_trees("footwear", features)
#
...
#
#         # pred_classes = predictions[0]["predictions"]
#         # class_to_pick = 5
#         # preds_one_class = (pred_classes == class_to_pick).nonzero().flatten().tolist()
#         # random.seed(512313)
#         # random.shuffle(preds_one_class)
#         # sample_preds = preds_one_class[0:25]
#         # visualize_indices_subplots(sample_preds, dataset, class_to_pick)
#     else:
#         raise NotImplementedError
...
#
#     u_foot = load_ann(
#         "/home/ITRANSITION.CORP/i.sechko/PycharmProjects/Unsupervised-Classification/annoy_trees/footwear_dot_512.ann")
#
#     import matplotlib
#     matplotlib.use("Agg")
#     sums = []
#     ids = []
#     for i in range(0, 45000, 20):
#         _, dists = u_foot.get_nns_by_item(i, 25, include_distances=True)
#         sums.append(sum(dists))
#         ids.append(i)
#
...
#     #
#     #     min_sum_id = sums.index(min_sum)
#     #     sums[min_sum_id] = 100000
#     #
#     #     vector_id = ids[min_sum_id]
#     #
#     #     nns_ids, dists = u_foot.get_nns_by_item(vector_id, 25, include_distances=True)
#     #
#     #     print("Distances:", dists)
#     #     visualize_indices_subplots(nns_ids, dataset, f"2260dot/{i}_{vector_id}_{min_sum}")
#
#     nns_ids, dists = u_foot.get_nns_by_item(26840, 25, include_distances=True)
#
#     print("Distances:", dists)
#     visualize_indices_subplots(nns_ids, dataset, f"test")
