...
def get_train_dataset(p, transform, to_augmented_dataset=False,
    ...
    if p['train_db_name'] == 'custom':
        ...
        dataset = CustomDataset(image_dir=p["train_image_dir"], transform=transform)
...
def get_val_dataset(p, transform=None, to_neighbors_dataset=False):
    # Base dataset
    if p['val_db_name'] == 'custom':
        from data.custom_dataset import CustomDataset
        dataset = CustomDataset(image_dir=p["val_image_dir"], transform=transform)
