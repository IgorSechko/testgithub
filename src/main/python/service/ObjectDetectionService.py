...
from src.main.python.common import label_map_util

plt.switch_backend('agg')


#config = json.loads(open('../etc/directory_conf.json', 'r').read())
API_PATH = os.path.join('/home/ITRANSITION.CORP/s.shevkun/', 'models/research')
...
class ObjectDetector:
    ...
    def image_dir_description(self, image_urls, batch_size=50):
        # get list of images with batch
        batches = (np.arange(len(image_urls)) / batch_size).astype(np.uint16)
        return [
            {
                'filename': image_urls[file_num],
    ...
    def _prepare_img_arrays(self, img_dir, batches):
        ...
        print("length of array %s" % len(img_array))
...
if __name__ == '__main__':
    ...
    model_graph=config['root_dir'] + '/output_inference_graph.pb',
    label_path=config['root_dir'] + '/data_dir/annotations/label_map.pbtxt')
    detector.object_detection(image_dir=config['root_dir'] + '/data_dir/images')
